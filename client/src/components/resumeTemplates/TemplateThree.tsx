...
const TemplateThree: React.FC<TemplateThreeProps> = ({
  ...
  const generateSections = (): Section[] => {
    ...
    sections.push(
      createSection(
        ...
        <div>
          ...
          <h2
            className={`${styles.jobRoleSize} font-semibold capitalize `} // I will change this to latter
          ...
          <div className="flex gap-4">
            ...
            <div className="flex items-center gap-1">
              ...
              <span className="text-gray-600">{placeholderText.location}</span>
            </div>
          </div>
        </div>
      )
    ...
    if (activeSections.socialLinks && data.socialLinks?.length) {
      sections.push(
        createSection(
          "socialLinks",
          <div
            className={`flex flex-wrap gap-3 gap-y-0.5 mb-1 ${styles.fontSize.body}`}
          >
      ...
      sections.push(
        createSection(
          ...
          <section className={`${styles.sectionSpacing}`}>
            ...
            {/** It had mb-4 */}{" "}
            <h3
              className={` font-bold uppercase border-b border-gray-200 pb-1 mb-0.5 ${styles.fontSize.title}`} //It had text-lg
              style={{ color: primaryColor }}
            >
              Summary
            </h3>
            {/* <p className={`text-gray-600 ${styles.fontSize.body}`}>
    ...
    // Skills section
    if (activeSections.selectedSkills && data.selectedSkills?.length) {
      sections.push(
        createSection(
          "skills",
          <section className={`${styles.sectionSpacing}`}>
            ...
            <div className="flex flex-wrap gap-2">
              {data.selectedSkills.map((skill, index) => (
                <span
                  key={index}
                  className={`px-2 py-0 bg-gray-100 ${styles.skillsFontSizes} rounded-[3px] border-[0.1px] text-gray-600`}
                >
          ...
          <section className={`${styles.sectionSpacing}`}>
            ...
            {data.workExperience.map((exp, index) => (
              <div key={index} className="mb-2">
                <h4
                  className={`font-bold ${styles.sectionInnerTitle}`} // text-base
                  // style={{ color: primaryColor }}
                >
                ...
                <div
                  className={`mt-1 text-gray-600 resumeList  ${styles.fontSize.body}`}
                  dangerouslySetInnerHTML={{
            ...
            {data.projects.map((project, index) => (
              <div key={index} className="mb-4">
                ...
                {project.links && project.links.length > 0 && (
                  <div className={`mt-1.5 text-[15px] flex flex-col gap-y-0.5`}>
    ...
    if (activeSections.customSections && data.customSections?.length) {
      sections.push(
        createSection(
          "customSections",
          <section className={`${styles.sectionSpacing}`}>
            ...
            {(data.education || []).map((edu, index) => (
              <div key={index} className={`${styles.sectionSpacing}`}>
                ...
                <div
                  className={`text-gray-600 mt-1 resumeList ${styles.fontSize.body}`}
                  dangerouslySetInnerHTML={{
                    __html: edu.description || "Enter description",
                  }}
                />
              </div>
            ))}
    ...
    if (
      (activeSections.education && data.education?.length) ||
      (activeSections.certificate && data.certificate?.length) ||
      (activeSections.languages && data.languages?.length) ||
      (activeSections.awards && data.awards?.length)
    ) {
      sections.push(
        createSection(
          "bottomSections",
          <div
            className={`
              ...
              ${activeThreeSections === 4 ? "grid-cols-3" : "grid-cols-3"}
            `}
          >
            {activeSections.education && (
              <section
                className={`
                  col-span-1
                  ${activeThreeSections === 4 ? "hidden" : "hidden"}
                `}
              >
                <h3
                  className={`font-bold uppercase border-b border-gray-200 pb-1 ${styles.sectionSpacing} ${styles.fontSize.title}`}
                  style={{ color: primaryColor }}
                >
                  Education
                </h3>
            ...
            <div
              className={`flex gap-6 ${
                activeThreeSections === 4 ? "col-span-3" : "col-span-3"
              }`}
            >
              {/* Additional Sections */}
              {activeSections.education && (
                <section
                  className={`
                    col-span-1 w-full
                    ${activeThreeSections === 4 ? "hidden" : "col-span-1 mb-4"}
              ...
              {activeSections.certificate && (
                <section className={` ${styles.sectionSpacing} w-full`}>
                  <h3
                    className={`font-bold uppercase border-b border-gray-200 pb-1 mb-2 ${styles.fontSize.title}`}
                    style={{ color: primaryColor }}
                  >
                  ...
                  {data.certificate.map((cert, index) => (
                    <div key={index} className="mb-3">
                      ...
                      {/* <p
                        className={`text-gray-600 mt-1 ${styles.fontSize.body}`}
                      >
  ...
  return (
    <div className="flex flex-col gap-8" ref={mainContentRef}>
      {/* First Page */}
      <div
        className={`w-[794px] aspect-[1/1.414] h-[1123px] shadow bg-white ${styles.pageMargin} overflow-hidden mx-auto  ${styles.fontFamilies} ${styles.lineHeight}`}
      >
