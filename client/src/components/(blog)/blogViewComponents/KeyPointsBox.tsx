...
const KeyPointsBox = ({ items }: { items: string[] }) => {
  return (
    <Card className="bg-white shadow rounded border border-gray-200 p-8 max-lg:p-5 max-md:p-4 max-w-4xl mx-auto my-4">
      ...
      <ul className="space-y-4">
        {items.map((point, index) => (
          <li key={index} className="flex items-start gap-3 group">
            ...
            <span className="text-gray-700 text-muted-foreground text-[18px]  font-blogText leading-relaxed group-hover:text-gray-900 transition-colors duration-300">
              {point}
            </span>
          </li>
        ))}
