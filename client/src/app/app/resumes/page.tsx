...
//       if (response.data.success) {
//         console.log("response.data for fetchAllResumes one two", response.data);
...
//       fetchAllResumes();
//     }
//   }, [isLoading, user?._id, fetchAllResumes]);
...
//     return allResumes.reverse().map((resume) => {
//       if (!resume) return <EmptyResumeCard key="empty" />;
//       const mappedData = mapMongoDataToReduxFormat(resume);
//       if (!mappedData) return <EmptyResumeCard key="empty-mapped" />;
//       console.log("default resume", resume);
//       const initialResume = mappedData;
//       console.log("initialResume for grid", initialResume);
//       return (
//         <div
//           key={resume._id}
//           className="w-full max-w-[420px] h-[583px] bg-white border-d border-gray-200 rounded shadow-md p-3 m-4 cursor-pointer aspect-[1/1.41]"
//         >
...
//               //   handleResumeTitleUpdate(title, resume._id as string)
//               // }
//               // createdAt={resume.}
//             />

//             {/* this one is for the overlay */}
//             <div className="w-full h-full absolute left-0 top-0 opacity-80 z-30"></div>

//             <ResumeViewer resumeId={resume._id} resume={initialResume} />
//           </div>
//         </div>
//       );
//     });
//   }, [allResumes, ResumeViewer, user?._id, !isLoading]);

//   const handleResumeDelete = useCallback(async () => {
//     try {
//       if (!resumeIdToDelete || resumeIdToDelete.length !== 24) return;
//       if (!user?._id) return;
//       const response = await api.post("/resume/delete-resume", {
//         resumeId: resumeIdToDelete,
//         userId: user?._id,
//       });
//       console.log("response.data for handleResumeDelete", response.data);
//       if (response.data.success) {
...
//     }
//   }, [resumeIdToDelete, fetchAllResumes, !isLoading, user?._id]);
...
//       }
//     },
//     [fetchAllResumes, !isLoading, user?._id]
//   );
...
// // export default MyResume;"use client";
// "use client";
// import React, { useEffect, useState, useMemo, useCallback } from "react";
// import AppHeader from "@/components/AppHeader";
// import DeleteResumeModal from "@/components/resume/DeleteResumeModal";
...
//   <div className="w-full max-w-[418px] h-[618px] bg-white border-d border-gray-200 rounded shadow-md p-3 m-4 cursor-pointer">
//     <div className="w-full h-full bg-gray-100 rounded flex items-center justify-center">
//       <span className="text-gray-400">Create a Resume</span>
//     </div>
//   </div>
// ));
...
//     // Only show empty state when we're sure there are no resumes
//     if (!isDataLoading && allResumes.length === 0) {
//       return <EmptyResumeCard />;
//     }
...
//       );
//     });
//   }, [allResumes, ResumeViewer, user?._id, isDataLoading]);
...
//       console.error("Error deleting resume:", error);
//     }
//   }, [resumeIdToDelete, fetchAllResumes, user?._id]);
...
//       className="w-full max-w-[418px] h-[618px] bg-white border border-gray-200 rounded-lg shadow-sm hover:shadow-md transition-all duration-300 p-3 m-4 cursor-pointer group relative overflow-hidden"
//     >
...
//     // Always show the EmptyResumeCard after existing resumes
//     return [...resumes, <EmptyResumeCard key="empty-card" />];
//   }, [allResumes, ResumeViewer, user?._id, isDataLoading]);
...
const EmptyResumeCard = React.memo(() => {
  ...
  return (
    <div
      onClick={handleCreateResume}
      className="w-full max-w-[420px] h-[583px] bg-white border border-gray-200 rounded-lg shadow-sm hover:shadow-md transition-all duration-300 p-3 m-4 cursor-pointer group relative overflow-hidden"
    >
      <div className="w-full h-full bg-gradient-to-br from-gray-50 to-gray-100 rounded-lg flex flex-col items-center justify-center space-y-4 transition-all duration-300 group-hover:bg-gradient-to-br group-hover:from-blue-50 group-hover:to-blue-100">
        {isCreating ? (
          <>
